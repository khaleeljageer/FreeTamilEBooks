...
@Entity(
    ...
    foreignKeys = [
        ForeignKey(
            entity = EBook::class,
            parentColumns = [EBook.ID],
            childColumns = [EBookHighlight.BOOK_ID],
            onDelete = ForeignKey.CASCADE
        )
    ],
    indices = [Index(value = [EBookHighlight.BOOK_ID])]
)
data class EBookHighlight(
...
class HighlightConverters {
    @TypeConverter
    fun styleFromString(value: String?): EBookHighlight.Style =
        EBookHighlight.Style.getOrDefault(value)

    @TypeConverter
    fun styleToString(style: EBookHighlight.Style): String = style.value
